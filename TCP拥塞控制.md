...
**2.快速**
    这样说来，等待超时重传并不是特别好的办法，那怎么办呢？我们来看一张简略时序图，更容易说明什么是快速重传。
发送方                         接收方
M1 <----------------------     M1 ACK
M2 <----------------------     M2 ACK
M3 ----------------------->    lost
...
M3 ----------------------->    M3 
   ...
   在这张图中，发送方的M1,M2包都收到了接收方发回的ACK确认，但M3丢失。因为M3的超时计时器T3未到时，所以发送方不会理解重传M3，这时继续发送M4,M5,M6。对于接收方来讲，无论是M4，M5或者M6，它都不会返回对于的ACK确认，因为它发现序号不对，它认为它只收到过M1,M2，接下来它应该收到M3才对，你发送方发过来的M4,M5,M6老子不要，所以它继续不断给发送方返回M2的ACK确认，目的是告知发送方它只收到M2。
